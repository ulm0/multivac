...
- **A red `verify` now reaches the agent in Claude Code, and after an edit the
  agent has to read it.** `doors` now writes `mvac verify 2>&1 || true` at
  session start, where nothing may block, and `mvac verify >&2 || exit 2` after
  an edit, where a red law, a config the edit broke and a missing binary all
  come back to the model. This is a forced read in the same turn, not a revert.
  Re-running `doors` rewrites an existing bare `mvac verify` entry in place, and
  a `mvac verify --strict` you wrote stays yours. Only the `claude` target
  carries a hook config, and a brain that never re-runs `doors` keeps the old,
  mute command. (MV-112)
- **A commit that deletes the law is refused.** A row that is `active` at HEAD
  and absent from the index is now refused with exit 1, and so is an index that
  removes the law file. Retiring a row stays allowed, and a `proposed` row may
  still disappear. Where the question cannot be answered — no HEAD, an
  unreadable index, a consumer checkout — the check says so and does not gate.
  (MV-107) **Retired rows are covered too**: deleting one is refused. (MV-117)
- **An unknown config key or `--sdd` name is refused by name.** A key multivac
  does not know is refused, naming the key it is near when only case or
  separators differ and listing the known keys otherwise; `--sdd=` exits 2
  instead of 1. **If you run more than one version**: from this release on, a
  key a newer multivac knows is refused by an older one. And `init --grapher` is
  still not checked: an unknown name is written to the config, and `doors` and
  `doctor` report it as not verified. (MV-114)
- **Exit codes keep the documented contract in four more commands.** `seed`,
  `repos`, `repos sync` and `roadmap sync` exited 1 on a config that will not
  load; they exit 2 now. `doors` and `doctor` remain the documented exceptions;
  `init`, which now refuses a broken config, exits 1 too, though the reference
  does not list it; and bare `roadmap` still exits 0 on a broken config because
  it never reads one. **Bare `doctor` now exits 1 when an anchor in the law does
  not parse**, and prints a `law` line naming them. (MV-118, amending MV-85)

**Fixed**

- **`--flag=value` is accepted again.** A long flag written with `=` is matched
  by its name now. A declared valued flag whose value is missing, or is itself a
  flag, is refused naming the flag. And `change` and `count` read the same
  refusal as every other command, so `change land <slug> api` no longer exits 0
  having recorded nothing. (MV-105)
- **The commit gates read the commit being made, `git commit -a` included.**
  Under `-a` they saw fewer paths than the commit contained, which walked the
  enactment and config checks straight past the most common way to commit; under
  a pathspec commit they saw more. The index git hands the hook is honoured now,
  for the repository the hook runs in and no other. (MV-106)
- **multivac no longer takes "it is there" to mean "it is ours".** `doors` wrote
  a stub door such as `.github/copilot-instructions.md` whole; it writes only
  its managed block there now, and frontmatter only when it creates the file.
  The commit hook's first choice of runner — the repository's own build — now
  requires the repository's `package.json` to name multivac. A hook that
  mentions multivac only on a comment line no longer reads as wired; a mention
  inside a quoted string on a real line still reads as wired, by design.
  multivac's own shim beside husky is recognised and rewritten, so
  `strict_pre_push` arms there. And `init .` no longer downgrades a strict
  pre-push shim to a plain one. A hand-edit to a hook multivac wrote is still
  lost when the hook is regenerated. (MV-108)
- **Hooks and projections survive the environments they land in.** In a linked
  worktree the repository's own lefthook or hand-written gate never ran; the
  shim looks under `--git-common-dir` now. A declared grapher refresh ran
  through a shell after an edit but was split on spaces at `change close`; both
  run it through a shell. One mangled managed file no longer ends a multi-repo
  run: that file gets a notice naming it, and a file with two marker pairs is
  refused rather than half updated. `doctor` reads a shim instead of checking
  that it exists. (MV-115)
- **A broken config no longer disarms a strict gate through `init`, and a
  commented `requires:` floor is read.** `init` now refuses a config it cannot
  load, with exit 1, though it may already have run `git init` in a directory
  that was not a repository. And `requires: ">=0.4.0" # floor for CI` was
  ignored; a trailing comment no longer hides the floor, so a binary below it
  gets its notice, and a malformed floor with a comment is named in its own
  notice. Neither notice changes an exit code. (MV-114)
- **The anchor engine reads one way.** A bare `[:digit:]` outside a bracket
  expression is refused now — an `absent` leg written that way was green
  forever. The dialect gate also refuses `(?…)`, lazy quantifiers,
  backreferences, `\0` and alphabetic escapes with no ERE meaning. A line in a
  CRLF file is a line — a `$`-anchored or exact-line leg never matched there; a
  lone `\r` is still text. `count` reads the bytes `verify` reads, so a
  `count=N` pinned from it agrees with the gate. (MV-109)
- **Self-heal stays inside the file kind a leg names, and a symlink is not file
  text.** A `present` leg whose pattern moved is healed by rewriting its glob,
  and a candidate must now share the include's trailing extension; an include
  with no trailing extension, or ending in a brace group, keeps only the
  `.multivac/` fence. Tracked symlinks and gitlinks are listed by neither
  reader. A leg whose glob names only a symlink now matches no tracked file;
  point it at the link's target. (MV-116)
- **An SDD proof names exactly one feature.** The spec-kit proof was
  `specs/*<slug>*/spec.md`. The artifact language has no wildcard now: `<n>` is
  one run of digits, so the proof is `specs/<n>-<slug>/spec.md` and an openspec
  archive is `<n>-<n>-<n>-<slug>`. When more than one directory proves one step,
  the gate refuses and names them. A directory of the right shape written by
  hand still proves the step. (MV-113)
- **The lifecycle commits what it wrote and reports what happened.**
  `change land --landed` commits its status bump now. `--abandon` names repos
  that already landed instead of printing *nothing landed*. `change new` refuses
  a slug whose archive already exists. `roadmap sync` passed `--label` to
  `gh issue edit`, so every GitHub update failed; it passes `--add-label` now,
  and a failed update or close is printed as *NOT updated* or *NOT closed*,
  followed by the command that failed, and a failed update still prints the
  *not found in the tracker* line after it. A GitHub issue still accumulates
  status labels, and `close`'s spec-kit ledger still passes when the artifact
  never existed. (MV-110)
- **`change close` loses nothing on the way to the archive.** It verified claims
  against every anchor it could see, including anchors written inside the change
  file, then archived that file — which the parser never reads. It refuses now,
  naming the claim. Two branches closing the same slug could overwrite an
  archived record; the archive write itself refuses now. And a frontmatter key
  the lifecycle does not know, which its next rewrite of the file drops, is
  named whenever a command reads the change file, as a notice rather than a
  refusal. `--abandon` still reports rather than refuses over landed repos, on
  purpose. (MV-117)
- **A law row whose statement contains a pipe is read correctly.** The trailing
  columns are read from the end of the row now, and a row missing them claims
  nothing rather than borrowing a neighbour's cell. (MV-119)
- **`init` tells you to commit what it wrote, and the skill stops teaching what
  the tool does not do.** A fresh brain's first `change new` is refused until
  the scaffold is committed; `init`'s closing steps now start with that commit.
  The shipped skill taught an arrow-edge `landing_order` the parser refuses,
  told the interview to write into the brain door's managed block, which `doors`
  regenerates whole on every run, and said `change apply` re-projects doors,
  which it does not. All three are corrected. (MV-111) The site's session-zero
  guide gave the same managed-block advice and is corrected too. (MV-118)
- **The brain door no longer implies the graph stays uncommitted.** Its line for
  a declared grapher said "never staged or committed by multivac"; it now reads
...
- In multivac's own repository, the committed copy of the skill its harness
  ...
  package ships by a test — same files, same bytes. (MV-72)
