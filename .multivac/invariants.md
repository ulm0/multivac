...
| MV-81 | **Who enacts a row is not a fact on disk.** The rule that an agent proposes and a human enacts governs who may change the law, and `verify` cannot check it: multivac never fabricates git identity (MV-04), so an agent commits as the person — seven rows went `proposed → active` in this repo with the human's name on every commit — and git hooks execute with the caller's permissions, so a gate installed at pre-commit is one the same process can skip. A guardrail cannot live on the side of the thing it stops. The row therefore declares itself **ungateable with its reason**, the shape this project already uses for a step no artifact can prove, and names where enforcement actually is: the forge's merge button, held by an account the agent does not have. The half that IS checkable is not who but when — a state change to `active` lands in its own commit, never beside the code it anchors, because a rule and its evidence arriving together under one hand is a rule nobody reviewed on its own. | open | proposed | 2026-08-16 | [changes/enactment-is-gated-where-the-credential-lives.md](changes/enactment-is-gated-where-the-credential-lives.md) |
