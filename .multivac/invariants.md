...
| MV-29 | The site names no flag the binary does not accept. `doors` takes no flags at all — `--no-symlink` was documentation-only and never parsed — so the string appears nowhere in the site or the source. **Strengthened 2026-08-17 (MV-85):** "takes no flags" used to mean *ignores them*, and this row's leg proved it by pinning the unused `_argv` parameter — the absence of a read standing in for the absence of a surface. `doors` now declares an empty surface and REFUSES any argument, so the leg moved to that declaration: the positive statement rather than the mechanism that happened to imply it. The row is not relaxed by the move, it is the same claim proven by better evidence, and a reader who passes `--no-symlink` today is told so instead of being ignored. | specified | active | 2026-08-13 | [DESIGN.md](../DESIGN.md) |
...
<!-- @anchor MV-76 brain:src/adapters/sdd.ts /new RegExp\(doc\.placeholder\)\.test\(found\.body\)/ -->
...
<!-- @anchor MV-85 brain:src/commands/doors.ts /undeclared\('doors', argv, \{\}\)/ unique -->
...
| MV-86 | RESERVED by change the-brain-knows-what-projected-it — state the rule here before close. | open | proposed | 2026-08-17 | [changes/the-brain-knows-what-projected-it.md](changes/the-brain-knows-what-projected-it.md) |
