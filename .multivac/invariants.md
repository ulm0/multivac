...
| MV-82 | **A leg skips a line only when that line carries an anchor comment.** What the guard is FOR: an anchor line carries its own regex TEXT, so when a leg scans a file that holds anchors — this table, a change file, DESIGN.md, the guide page teaching the grammar, or a test fixture built from anchor strings — that text must not satisfy or break another leg. What it must NOT become: a substring test for the word `@anchor`. That was its reach until this row, and it meant every line of every file of every type that merely mentioned the word was invisible to every leg — so `const evade = "user.name"; // @anchor` in `src/` passed MV-04's tombstone with exit 0 while the same line without the seven-character suffix broke it. For a tool whose only value is that a gate ran, a keyword that silences any `absent`, `unique`, `count` or `each!` leg per line is the worst defect available, and it is easiest to reach for exactly when a gate is in the way. The predicate is therefore ONE exported recogniser, `ANCHOR_LINE`, shared by `parseAnchors` — which decides a line declares a leg — and by `matchesInFile` — which decides a line contributes no matches. Two private copies is how the reaches diverged; sharing one is what stops them diverging again. The literal carries no `g` flag, so no line's verdict depends on the line tested before it. **The ceiling, stated rather than discovered**: a line forging the comment form inside source code still hides itself. No test on a line's shape can tell a forged instruction from a quoted one, and the narrower reading that would catch it — requiring the comment to begin the line — un-hides ~100 fixture lines in `test/verify/` that quote whole anchors inside string literals, reopening at scale the false-report class the guard exists to prevent. What this row removes is the case an ordinary word in an ordinary comment reaches by accident. **This row carries no tombstone, and the reason is the defect's own signature**: a leg `brain:src/anchor/match.ts /includes\('@anchor'\)/ absent` was drafted, validated at 0 matches, then withdrawn on measurement — under the reverted code the offending line contains `@anchor` and therefore hides itself from every leg, so the tombstone reads green in both states and can never fire. It would have been decoration. The three positive legs are what bite: reverting the scanner makes MV-82's `ANCHOR_LINE.test(lines[i])` leg find nothing. | specified | proposed | 2026-08-17 | [changes/anchor-line-not-substring.md](changes/anchor-line-not-substring.md) |
